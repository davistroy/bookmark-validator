...
class TestValidationUtilities:
    ...
    @pytest.mark.parametrize("url,expected", [
        ...
        ("mailto:test@example.com", False),  # Should be invalid for bookmark processing
...
class TestCLIValidator:
    ...
    def test_validate_file_compatibility(self, temp_csv_file, temp_dir):
        ...
        errors = validator.validate_file_compatibility(str(temp_csv_file), str(temp_csv_file))
        assert len(errors) > 0
        assert "same file" in errors[0].lower()
        
        # Different files should be valid
        errors = validator.validate_file_compatibility(str(temp_csv_file), str(output_file))
        assert len(errors) == 0
