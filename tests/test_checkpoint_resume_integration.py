...
from bookmark_processor.core.checkpoint_manager import CheckpointManager, ProcessingStage, ProcessingState
...
"""
    ...
    async def test_checkpoint_creation_during_processing(self, checkpoint_config):
        ...
        with patch.object(pipeline.url_validator, 'batch_validate') as mock_validate, \
            ...
            state = pipeline.checkpoint_manager.load_checkpoint(checkpoint_config.input_file)
    ...
    async def test_resume_from_url_validation_stage(self, checkpoint_config):
        ...
        with patch.object(pipeline.url_validator, 'batch_validate') as mock_validate, \
            ...
            assert "https://docs.python.org/tutorial" not in call_args  # Already validated
    ...
    async def test_resume_from_ai_processing_stage(self, checkpoint_config):
        ...
        for url in urls:
            validation_result = ValidationResult(url=url, is_valid=True, status_code=200)
            mock_bookmark = Mock()
        ...
        with patch.object(pipeline.url_validator, 'batch_validate') as mock_validate, \
            ...
            original_count = sum(1 for r in pipeline.ai_results.values() 
                               if r.processing_method == "test_ai")
            resumed_count = sum(1 for r in pipeline.ai_results.values() 
    ...
    @pytest.mark.asyncio
    async def test_checkpoint_data_integrity_after_interruption(self, checkpoint_config):
        ...
        with patch.object(pipeline.url_validator, 'batch_validate') as mock_validate, \
            ...
            validation_results = [
                ValidationResult(url="https://docs.python.org/tutorial", is_valid=True, status_code=200),
                ValidationResult(url="https://arxiv.org/abs/12345", is_valid=True, status_code=200),
                ValidationResult(url="https://developer.mozilla.org/js", is_valid=False, status_code=404),
            ]
            mock_validate.return_value = validation_results
        ...
        assert len(state.ai_results) >= 0  # May be 0 if interrupted before AI processing
    ...
    async def test_multiple_resume_cycles(self, checkpoint_config):
        ...
        with patch.object(pipeline1.url_validator, 'batch_validate') as mock_validate1, \
        ...
        with patch.object(pipeline2.url_validator, 'batch_validate') as mock_validate2, \
            ...
            first_resume_count = sum(1 for r in pipeline2.ai_results.values() 
                                   if r.processing_method == "first_resume")
            second_resume_count = sum(1 for r in pipeline2.ai_results.values() 
...
class TestCheckpointPerformanceAndScaling:
    ...
    def large_csv_data(self):
        ...
        lines = ['id,title,note,excerpt,url,folder,tags,created,cover,highlights,favorite']
        ...
        with patch.object(pipeline.url_validator, 'batch_validate') as mock_validate, \
            ...
            for i in range(20):
                validation_results.append(
                    ValidationResult(url=f"https://example.com/{i}", is_valid=True, status_code=200)
                )
            mock_validate.return_value = validation_results
            ...
            if len(checkpoint_saves) > 1:
                intervals = [checkpoint_saves[i+1] - checkpoint_saves[i] 
    ...
    async def test_checkpoint_memory_efficiency(self, performance_config):
        ...
        for _, row in df.iterrows():
            ...
            ai_results[url] = {
                'original_url': url,
                'enhanced_description': f"Enhanced description for {row['title']} with lots of content " * 10,
        ...
        start_size = sum(f.stat().st_size for f in Path(performance_config.checkpoint_dir).glob("*"))
        
        checkpoint_manager.save_checkpoint(force=True)
        
        end_size = sum(f.stat().st_size for f in Path(performance_config.checkpoint_dir).glob("*"))
        checkpoint_file_size = end_size - start_size
        
        # Verify reasonable file size (should be compressed)
        assert checkpoint_file_size > 0
        assert checkpoint_file_size < 1024 * 1024  # Should be less than 1MB for 20 bookmarks
    ...
    async def test_concurrent_checkpoint_access(self, performance_config):
        ...
        loaded_state = checkpoint_manager2.load_checkpoint(performance_config.input_file)
