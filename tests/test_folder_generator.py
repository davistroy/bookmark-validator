...
class TestFolderNode:
    ...
    def test_add_bookmark(self):
        ...
        bookmark = Bookmark(
            url="https://python.org", title="Python.org", created=datetime.now()
        )
    ...
    def test_get_bookmark_count(self):
        ...
        dev.add_bookmark(
            Bookmark(url="https://dev.com", title="Dev", created=datetime.now())
        )
        dev.add_bookmark(
            Bookmark(url="https://code.com", title="Code", created=datetime.now())
        )
        python.add_bookmark(
            Bookmark(url="https://python.org", title="Python", created=datetime.now())
        )
...
class TestAIFolderGenerator:
    ...
    def test_extract_domain(self, folder_generator):
        ...
        assert (
            folder_generator._extract_domain("https://github.com/user/repo")
            == "github.com"
        )
        assert (
            folder_generator._extract_domain("https://www.example.com/page")
            == "example.com"
        )
        assert (
            folder_generator._extract_domain("http://subdomain.site.com")
            == "subdomain.site.com"
        )
        assert folder_generator._extract_domain("invalid-url") == ""

    def test_determine_category(
        self, folder_generator, sample_bookmarks, content_data_map
    ):
    ...
    def test_generate_folder_structure_basic(
        self, folder_generator, sample_bookmarks, content_data_map
    ):
        ...
        assert all(
            url in result.folder_assignments
            for url in [b.url for b in sample_bookmarks]
        )
    ...
    def test_hierarchical_structure(
        ...
        result = folder_generator.generate_folder_structure(
            sample_bookmarks, content_data_map=content_data_map
        )

        # Check for hierarchical paths
        has_hierarchical = any(
            "/" in path for path in result.folder_assignments.values()
        )
    ...
    def test_fallback_category(self, folder_generator):
        ...
        bookmark = Bookmark(
            url="https://random-site.com", title="Random Site", created=datetime.now()
        )
        ...
        category = folder_generator._fallback_category(
            bookmark, "MySpecialFolder/Subfolder"
        )
    ...
    def test_group_similar_bookmarks(self, folder_generator):
        ...
        bookmarks = [
            Bookmark(
                url="https://github.com/repo1", title="Repo 1", created=datetime.now()
            ),
            Bookmark(
                url="https://github.com/repo2", title="Repo 2", created=datetime.now()
            ),
            Bookmark(
                url="https://github.com/repo3", title="Repo 3", created=datetime.now()
            ),
            Bookmark(
                url="https://stackoverflow.com/q1",
                title="Question 1",
                created=datetime.now(),
            ),
            Bookmark(
                url="https://stackoverflow.com/q2",
                title="Question 2",
                created=datetime.now(),
            ),
            Bookmark(
                url="https://example.com", title="Example", created=datetime.now()
            ),
        ]
...
class TestIntegrationWithPipeline:
    ...
    def test_folder_generation_result_serialization(self):
        ...
        sample_bookmarks = [
            Bookmark(
                url="https://github.com/test", title="Test Repo", created=datetime.now()
            )
        ]
