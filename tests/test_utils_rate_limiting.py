...
class TestRateLimiter:
    ...
    def test_wait_delay_needed(self):
        ...
        with patch("time.sleep") as mock_sleep:
            with patch(
                "time.time", return_value=limiter.last_request_time + 0.5
            ):  # 0.5 seconds later
                limiter.wait()
    ...
    def test_get_delay(self):
        ...
        with patch(
            "time.time", return_value=limiter.last_request_time + 0.25
        ):  # 0.25 seconds later
            delay = limiter.get_delay()
...
class TestIntelligentRateLimiter:
    ...
    def test_get_delay_max_limit(self):
        ...
        limiter = IntelligentRateLimiter(
            default_delay=1.0, max_delay=5.0, adaptive_factor=10.0
        )
        url = "https://example.com"
