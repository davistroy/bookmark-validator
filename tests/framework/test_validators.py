...
from bookmark_processor.core.data_models import ProcessingResults, Bookmark


class ValidationResult:
    """Result of a validation check."""
    
    def __init__(self, validator_name: str, passed: bool, message: str = "", details: Dict[str, Any] = None):
...
class ResultValidator(BaseValidator):
    ...
    def validate(
        ...
        passed = len(errors) == 0
        message = "All validations passed" if passed else f"Found {len(errors)} validation errors: {'; '.join(errors[:3])}"
    ...
    def _validate_processing_results(self, results: ProcessingResults) -> List[str]:
        ...
        # Logical validation
        if hasattr(results, 'processed_bookmarks') and hasattr(results, 'valid_bookmarks') and hasattr(results, 'invalid_bookmarks'):
            if results.valid_bookmarks + results.invalid_bookmarks != results.processed_bookmarks:
                errors.append("valid_bookmarks + invalid_bookmarks should equal processed_bookmarks")
    ...
    def _validate_output_file(self, output_file: Path, results: ProcessingResults) -> Tuple[List[str], Dict[str, Any]]:
        ...
        try:
            ...
            if actual_columns != expected_columns:
                errors.append(f"Output columns incorrect. Expected: {expected_columns}, Got: {actual_columns}")
            
            # Check row count consistency
            output_row_count = len(df)
            if output_row_count != results.valid_bookmarks:
                errors.append(f"Output row count ({output_row_count}) doesn't match valid_bookmarks ({results.valid_bookmarks})")
    ...
    def _validate_expected_results(self, results: ProcessingResults, expected: Dict[str, Any]) -> List[str]:
        ...
        if 'min_success_rate' in expected and results.total_bookmarks > 0:
            ...
            if actual_rate < expected['min_success_rate']:
                errors.append(f"Success rate {actual_rate:.2%} below minimum {expected['min_success_rate']:.2%}")
        
        # Check maximum duration
        if 'max_duration' in expected and results.processing_time > expected['max_duration']:
            errors.append(f"Processing time {results.processing_time:.2f}s exceeds maximum {expected['max_duration']:.2f}s")
        
        # Check should complete flag
        if expected.get('should_complete', True) and results.processed_bookmarks != results.total_bookmarks:
            errors.append(f"Processing incomplete: {results.processed_bookmarks}/{results.total_bookmarks} processed")
        
        # Check minimum processed count
        if 'min_processed' in expected and results.processed_bookmarks < expected['min_processed']:
            errors.append(f"Processed count {results.processed_bookmarks} below minimum {expected['min_processed']}")
        
        return errors
    
    def _calculate_validation_metrics(self, results: ProcessingResults) -> Dict[str, Any]:
    ...
    def validate(
        ...
        if expected_checkpoints is not None:
            if len(checkpoint_files) != expected_checkpoints:
                errors.append(f"Expected {expected_checkpoints} checkpoint files, found {len(checkpoint_files)}")
        ...
        passed = len(errors) == 0
        message = "Checkpoint validation passed" if passed else f"Found {len(errors)} checkpoint issues"
        ...
        try:
            ...
            for field in required_fields:
                if field not in checkpoint_data:
                    errors.append(f"Checkpoint file {checkpoint_file.name} missing required field: {field}")
            
            # Validate progress data
            if 'progress' in checkpoint_data:
                progress = checkpoint_data['progress']
                if not isinstance(progress, dict):
                    errors.append(f"Checkpoint file {checkpoint_file.name} has invalid progress format")
                else:
                    # Check for common progress fields
                    if 'processed_count' not in progress:
                        errors.append(f"Checkpoint file {checkpoint_file.name} missing processed_count in progress")
            ...
            if 'timestamp' in checkpoint_data:
                try:
                    ...
                    errors.append(f"Checkpoint file {checkpoint_file.name} has invalid timestamp format")
        
        except json.JSONDecodeError as e:
            errors.append(f"Checkpoint file {checkpoint_file.name} is not valid JSON: {str(e)}")
        except Exception as e:
            errors.append(f"Failed to validate checkpoint file {checkpoint_file.name}: {str(e)}")
    ...
    def _validate_resume_capability(self, checkpoint_files: List[Path]) -> List[str]:
        ...
        try:
            ...
            if 'progress' in checkpoint_data:
                ...
                missing_fields = [field for field in essential_fields if field not in progress]
                
                if missing_fields:
                    errors.append(f"Latest checkpoint missing essential resume fields: {missing_fields}")
...
class ErrorValidator(BaseValidator):
    ...
    def validate(
        ...
        if expected_error_types and results and results.errors:
            type_errors = self._validate_expected_error_types(results.errors, expected_error_types)
        ...
        passed = len(errors) == 0
        message = "Error validation passed" if passed else f"Found {len(errors)} error handling issues"
    ...
    def _validate_log_files(self, log_files: List[Path]) -> Tuple[List[str], Dict[str, Any]]:
        ...
        for log_file in log_files:
            ...
            try:
                ...
                error_indicators = ['ERROR', 'CRITICAL', 'EXCEPTION', 'Failed', 'Error:']
                file_error_count = sum(log_content.count(indicator) for indicator in error_indicators)
    ...
    def _validate_expected_error_types(self, error_list: List[str], expected_types: List[str]) -> List[str]:
    ...
    def _validate_error_handling_behavior(self, results: ProcessingResults) -> List[str]:
...
class CompositeValidator:
    ...
    def get_overall_result(self, validation_results: Dict[str, ValidationResult]) -> bool:
