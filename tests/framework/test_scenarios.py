...
class TestScenario(ABC):
    ...
    def validate_results(
        self, results: ProcessingResults, output_file: Path, environment
    ) -> List[str]:
...
class BasicProcessingScenario(TestScenario):
    ...
    def run_scenario(
        ...
        return processor.process_bookmarks(
            input_file=str(input_file), output_file=str(output_file), **self.config
        )
    ...
    def validate_results(
        ...
        if results.total_bookmarks > 0:
            ...
            if success_rate < self.expected_results["min_success_rate"]:
                errors.append(
                    f"Success rate {success_rate:.2f} below minimum {self.expected_results['min_success_rate']}"
                )
        ...
        if not output_file.exists():
            ...
            try:
                ...
                if list(output_df.columns) != expected_columns:
                    errors.append(
                        f"Output columns incorrect: {list(output_df.columns)}"
                    )
...
class CheckpointResumeScenario(TestScenario):
    ...
    def run_scenario(
        ...
        try:
            ...
            config_phase1["checkpoint_dir"] = str(
                environment.get_directory("checkpoints")
            )
            ...
            checkpoint_files = list(
                environment.get_directory("checkpoints").glob("*.json")
            )
            if checkpoint_files:
            ...
            config_phase2["checkpoint_dir"] = str(
                environment.get_directory("checkpoints")
            )
        ...
        if not output_file.exists():
            ...
            try:
                ...
                if len(output_df) != results.valid_bookmarks:
                    errors.append(
                        f"Output row count {len(output_df)} doesn't match valid bookmarks {results.valid_bookmarks}"
                    )
            except Exception as e:
...
class ErrorHandlingScenario(TestScenario):
    ...
    def validate_results(
        ...
        if len(results.errors) < self.expected_results["min_error_logging"]:
            errors.append(
                f"Expected at least {self.expected_results['min_error_logging']} errors, got {len(results.errors)}"
            )

        # Should have some valid bookmarks despite errors
        if results.valid_bookmarks < 2:
            errors.append(
                f"Expected at least 2 valid bookmarks, got {results.valid_bookmarks}"
            )

        # Should have some invalid bookmarks
        if results.invalid_bookmarks < 1:
            errors.append(
                f"Expected at least 1 invalid bookmark, got {results.invalid_bookmarks}"
            )
...
class PerformanceScenario(TestScenario):
    ...
    def setup_test_data(self, environment) -> Path:
        ...
        input_file = (
            environment.get_directory("input")
            / f"performance_test_{self.dataset_size}.csv"
        )
        df = pd.DataFrame(test_data)
        df.to_csv(input_file, index=False)
    ...
    def validate_results(
        ...
        if results.processing_time > self.expected_results["max_duration"]:
            errors.append(
                f"Processing took {results.processing_time:.2f}s, expected max {self.expected_results['max_duration']:.2f}s"
            )
        ...
        if results.total_bookmarks != self.dataset_size:
            errors.append(
                f"Expected {self.dataset_size} total bookmarks, got {results.total_bookmarks}"
            )
...
class ScenarioRunner:
    ...
    def run_scenario(
        ...
        try:
            ...
            output_file = (
                self.environment.get_directory("output") / f"{scenario.name}_output.csv"
            )
            ...
            # Store artifacts
            result.artifacts = {"input_file": input_file, "output_file": output_file}
        ...
        self.logger.info(
            f"Scenario {scenario.name} completed with status: {result.status}"
        )
        return result
    ...
    def _calculate_metrics(
        self, scenario: TestScenario, results: ProcessingResults
    ) -> Dict[str, Any]:
        ...
        if results:
            ...
            if results.total_bookmarks > 0:
                metrics["success_rate"] = (
                    results.valid_bookmarks / results.total_bookmarks
                )
                metrics["error_rate"] = (
                    results.invalid_bookmarks / results.total_bookmarks
                )
    ...
    def get_summary_report(self) -> Dict[str, Any]:
        ...
        completed_scenarios = sum(
            1 for r in self.results.values() if r.status == ScenarioStatus.COMPLETED
        )
        failed_scenarios = sum(
            1 for r in self.results.values() if r.status == ScenarioStatus.FAILED
        )
...
class StandardScenarios:
    ...
    def get_basic_scenarios() -> List[TestScenario]:
        ...
        return [BasicProcessingScenario(), ErrorHandlingScenario()]
