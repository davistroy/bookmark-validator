...
class DataGenerator:
    ...
    def generate_bookmark_data(
        self, count: int, include_invalid: bool = False, invalid_ratio: float = 0.1
    ) -> List[Dict[str, str]]:
    ...
    def generate_large_dataset(
        self, size: int, variation_seed: Optional[int] = None
    ) -> List[Dict[str, str]]:
    ...
    def _generate_bookmark_for_category(
        self, id_num: int, category: str
    ) -> Dict[str, str]:
...
class MockServiceManager:
    ...
    def setup_http_mock(
        ...
        def mock_request(*args, **kwargs):
            ...
            if random.random() < success_rate:
                ...
                response.url = (
                    args[0] if args else kwargs.get("url", "https://example.com")
                )
                ...
                if error_type == "connection_error":
                    ...
                    response.raise_for_status.side_effect = (
                        requests.exceptions.HTTPError("Mock HTTP error")
                    )
                    return response
    ...
    def setup_ai_mock(
        self, response_quality: str = "good", processing_delay: float = 0.1
    ) -> Mock:
            ...
            for bookmark in bookmarks:
                ...
                if response_quality == "good":
                    bookmark.enhanced_description = (
                        f"High-quality AI description for {bookmark.title}"
                    )
                    ...
                    if quality == "good":
                        bookmark.enhanced_description = (
                            f"Good AI description for {bookmark.title}"
                        )
...
class IntegrationTestFixtures:
    ...
    def create_test_dataset(
        ...
        data = self.data_generator.generate_bookmark_data(
            count=size, include_invalid=include_invalid
        )
    ...
    def create_performance_dataset(self, name: str, size: int) -> Path:
    ...
    def create_error_test_dataset(self, name: str) -> Path:
        ...
        valid_data = self.data_generator.generate_bookmark_data(
            count=5, include_invalid=False
        )
        invalid_data = self.data_generator.generate_bookmark_data(
            count=3, include_invalid=True, invalid_ratio=1.0
        )
    ...
    def setup_standard_mocks(
        self, network_success_rate: float = 0.9, ai_quality: str = "good"
    ) -> Dict[str, Mock]:
        ...
        mocks["http"] = self.mock_manager.setup_http_mock(
            success_rate=network_success_rate, response_delay=0.1
        )

        # AI mocking
        mocks["ai"] = self.mock_manager.setup_ai_mock(
            response_quality=ai_quality, processing_delay=0.05
        )
    ...
    def create_checkpoint_scenario(
        self, checkpoint_id: str, processed_count: int, total_count: int
    ) -> Path:
        ...
        checkpoint_file = (
            self.environment.get_directory("checkpoints") / f"{checkpoint_id}.json"
        )
    ...
    def verify_test_output(
        self, output_file: Path, expected_structure: bool = True, min_rows: int = 0
    ) -> Dict[str, Any]:
            ...
            if expected_structure:
                ...
                if results["columns"] == expected_columns:
                    ...
                    results["errors"].append(
                        f"Column structure incorrect: {results['columns']}"
                    )

            if results["row_count"] < min_rows:
                results["errors"].append(
                    f"Row count {results['row_count']} below minimum {min_rows}"
                )
