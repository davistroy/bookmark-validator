...
class MockValidator:
    ...
    def process_batch(self, items: List[str], batch_id: str) -> BatchResult:
        ...
        results = [
            ValidationResult(url=url, is_valid=True, status_code=200) for url in items
        ]
        return BatchResult(
            batch_id=batch_id,
...
class TestCostTrackingIntegration:
    ...
    def test_cost_tracking_disabled_by_default(self):
        ...
        config = BatchConfig()
        processor = EnhancedBatchProcessor(self.mock_validator, config)
    ...
    def test_cost_estimation_calculation(self):
        ...
        config = BatchConfig(enable_cost_tracking=True, cost_per_url_validation=0.001)
        processor = EnhancedBatchProcessor(self.mock_validator, config)
        processor.cost_tracker = self.cost_tracker
        ...
        assert (
            large_batch_cost.cost_factors["bulk_discount"] < 0
        )  # It's a negative discount
    ...
    def test_cost_factors_application(self):
        ...
        assert small_cost.total_estimated_cost == pytest.approx(
            small_cost.estimated_cost_per_item * 3, rel=1e-6
        )
        assert large_cost.total_estimated_cost == pytest.approx(
            large_cost.estimated_cost_per_item * 200, rel=1e-6
        )

    def test_budget_limit_enforcement(self):
        """Test budget limit checking."""
        config = BatchConfig(
            enable_cost_tracking=True, cost_per_url_validation=0.001, budget_limit=5.0
        )
        processor = EnhancedBatchProcessor(self.mock_validator, config)
        processor.cost_tracker = self.cost_tracker
        ...
        with patch("builtins.input", return_value="n"):
            result = processor._check_budget_and_confirm_sync(
                1.0
            )  # Would exceed 5.0 limit
            assert not result
    ...
    def test_cost_statistics_generation(self):
        ...
        assert (
            "average_batch_cost" in stats
        )  # Note: actual key is average_batch_cost, not average_cost_per_batch
    ...
    def test_budget_exceeded_with_user_override(self):
        ...
        config = BatchConfig(
            enable_cost_tracking=True, cost_per_url_validation=0.001, budget_limit=1.0
        )
        processor = EnhancedBatchProcessor(self.mock_validator, config)
        processor.cost_tracker = self.cost_tracker
    ...
    def test_failed_batch_cost_recording(self):
        ...
        processor = EnhancedBatchProcessor(failing_validator, config)
        processor.cost_tracker = self.cost_tracker
    ...
    def test_cost_factors_by_batch_size(self, batch_size, expected_factors):
        ...
        for factor in expected_factors:
            assert (
                factor in cost_breakdown.cost_factors
            ), f"Expected {factor} in {cost_breakdown.cost_factors}"
