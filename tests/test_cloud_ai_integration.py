...
def mock_config():
    ...
    config.get_rate_limit.return_value = 50
    config.get_batch_size.return_value = 10
    config.get_cost_tracking_settings.return_value = {
...
class TestCloudAIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_with_claude(self, mock_config, sample_bookmarks):
        ...
        # Mock the AI manager initialization and set up mock client
        with patch.object(ai_manager, "_initialize_clients"):
            await ai_manager.__aenter__()

            # Set up mock primary client after initialization
            ai_manager.primary_client = Mock()
            ai_manager.primary_client.generate_description = AsyncMock(
                return_value=(
                    "Test description generated by Claude",
                    {"provider": "claude", "success": True},
                )
            )
            ai_manager.primary_client.get_usage_statistics = Mock(
                return_value={
                    "provider": "claude",
                    "total_requests": 3,
            ...
            ai_manager.current_provider = "claude"
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_with_openai(self, mock_config, sample_bookmarks):
        ...
        with patch.object(ai_manager, "_initialize_clients"):
            ...
            ai_manager.primary_client.generate_description = AsyncMock(
                return_value=(
                    "Test description generated by OpenAI",
            ...
            ai_manager.current_provider = "openai"
    ...
    async def test_end_to_end_cloud_ai_workflow(self, mock_config, sample_bookmarks):
        ...
        # Mock initialization and set up mock client
        with patch.object(ai_manager, "_initialize_clients"):
            await ai_manager.__aenter__()

            # Set up mock primary client
            ai_manager.primary_client = Mock()
            ai_manager.primary_client.generate_description = AsyncMock(
                return_value=(
    ...
    def test_prompt_optimization(self):
        ...
        # Claude prompts should be reasonably sized and structured
        assert len(claude_prompt) < 700  # Allow for structured output format
        assert "Requirements:" in claude_prompt
        assert "description" in claude_prompt.lower()
        ...
        assert len(openai_messages[0]["content"]) < 300  # Concise system message
...
class TestPerformanceAndScaling:
    ...
    async def test_concurrent_request_handling(self):
        ...
        rate_limiter = RateLimiter(requests_per_minute=10, burst_size=10)
