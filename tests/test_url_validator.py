...
class TestValidationResult:
    ...
    def test_failed_result(self):
        ...
        result = ValidationResult(
            url="https://example.com", is_valid=False, error=error, attempts=3
        )
...
class TestURLValidator:
    ...
    def test_validate_single_url_timeout(self, validator):
        ...
        with patch.object(
            validator.session, "get", side_effect=Timeout("Request timeout")
        ):
            result = validator.validate_single_url("https://example.com")
    ...
    def test_validate_single_url_connection_error(self, validator):
        ...
        with patch.object(
            validator.session, "get", side_effect=ConnectionError("Connection failed")
        ):
            result = validator.validate_single_url("https://example.com")
    ...
    def test_validate_single_url_max_retries_exceeded(self, validator):
        ...
        with patch.object(
            validator.session, "get", side_effect=Timeout("Persistent timeout")
        ):
            result = validator.validate_single_url("https://example.com")
    ...
    def test_validate_single_url_rate_limiting(self, validator):
        ...
        with (
            patch.object(validator.session, "get", return_value=mock_response),
    ...
    def test_is_valid_url_scheme(self, validator):
        ...
        for url in valid_urls:
            assert (
                validator._is_valid_url_scheme(url) is True
            ), f"Should be valid: {url}"

        for url in invalid_urls:
            assert (
                validator._is_valid_url_scheme(url) is False
