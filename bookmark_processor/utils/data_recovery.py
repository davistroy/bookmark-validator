...
class DataRecoveryManager:
    ...
    def _init_default_strategies(self) -> Dict[str, DataRecoveryStrategy]:
        ...
        return {
            ...
            'title': DataRecoveryStrategy('derive', recovery_function=self._derive_title),
            ...
            'created': DataRecoveryStrategy('derive', recovery_function=self._derive_created_date),
    ...
    def handle_missing_data(self, field_name: str, record: Dict[str, Any]) -> Tuple[Any, List[str]]:
        """
        ...
        if not strategy:
            messages.append(f"No recovery strategy for field '{field_name}', using empty string")
            return '', messages
        ...
        if strategy.strategy == 'default':
            ...
            if strategy.recovery_function:
                ...
                messages.append(f"No recovery function for field '{field_name}', using default")
                return strategy.default_value, messages
            ...
            messages.append(f"Unknown recovery strategy '{strategy.strategy}' for field '{field_name}'")
            return strategy.default_value, messages
    
    def handle_malformed_data(self, field_name: str, value: Any, record: Dict[str, Any]) -> Tuple[Any, List[str]]:
        """
        ...
        sanitization_func = self.sanitization_functions.get(field_type, self._sanitize_string)
        ...
        try:
            ...
            if validation_result.is_valid:
                ...
                recovered_value, recovery_messages = self.handle_missing_data(field_name, record)
                messages.extend(recovery_messages)
                messages.append(f"Used recovery value for malformed field '{field_name}'")
    ...
    def recover_partial_record(self, partial_record: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
        """
        ...
        expected_fields = ['id', 'title', 'note', 'excerpt', 'url', 'folder', 'tags', 'created', 'cover', 'highlights', 'favorite']
        ...
        for field_name in expected_fields:
            if field_name in partial_record and partial_record[field_name] is not None:
                ...
                if validation_result.is_valid:
                    ...
                    recovered_value, field_messages = self.handle_malformed_data(field_name, value, partial_record)
                    recovered_record[field_name] = recovered_value
                    messages.extend(field_messages)
            else:
                # Field missing, handle accordingly
                recovered_value, field_messages = self.handle_missing_data(field_name, partial_record)
                recovered_record[field_name] = recovered_value
                messages.extend(field_messages)
...
class MalformedDataDetector:
    ...
    def detect_corruption(self, record: Dict[str, Any]) -> List[str]:
        ...
        # Title in URL field
        if url and not (url.startswith('http://') or url.startswith('https://')) and len(url) > 50:
            issues.append("Long text found in URL field (possible title)")
        
        # Check for repeated content across fields
        fields_content = {k: str(v).strip().lower() for k, v in record.items() if v and isinstance(v, str)}
...
def create_error_report(issues: List[str], record: Dict[str, Any], row_number: Optional[int] = None) -> str:
    """
    ...
    title = record.get('title', '')
    if title:
        report_parts.append(f"  Title: {title[:100]}{'...' if len(title) > 100 else ''}")
