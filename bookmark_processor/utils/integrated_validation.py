...
from .data_recovery import DataRecoveryManager, MalformedDataDetector, create_error_report
from .cli_validators import validate_cli_arguments
from .config_validators import validate_user_config, get_config_validation_report
...
class IntegratedValidator:
    ...
    def validate_application_startup(self, cli_args, config_path: Optional[Path] = None) -> Tuple[bool, List[str]]:
        """
    ...
    def validate_csv_data(self, csv_data: List[Dict[str, Any]], 
                         expected_columns: List[str],
                         recovery_mode: bool = True) -> Tuple[List[Dict[str, Any]], List[str]]:
        """
        ...
        for row_index, row_data in enumerate(csv_data):
            ...
            try:
                ...
                if row_result.is_valid:
                    ...
                    corruption_issues = self.malformed_detector.detect_corruption(row_data)
                    if corruption_issues:
                        self.logger.warning(f"Row {row_number} corruption detected: {', '.join(corruption_issues)}")
                    
                    # Attempt recovery
                    recovered_data, recovery_messages = self.data_recovery.recover_partial_record(row_data)
                    ...
                    if recovered_data and recovered_data.get('url'):
                        ...
                        recovery_summary = f"Row {row_number}: Data recovered successfully"
                        if recovery_messages:
                            recovery_summary += f" ({len(recovery_messages)} issues resolved)"
                        ...
                        error_report = create_error_report(
                            [issue.message for issue in row_result.get_errors()] + recovery_messages,
                            row_data,
                            row_number
                        )
    ...
    def validate_bookmark_record(self, bookmark_data: Dict[str, Any], 
                                record_id: Optional[str] = None) -> ValidationResult:
        """
    ...
    def validate_and_recover_bookmarks(self, bookmarks: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        ...
        for i, bookmark in enumerate(bookmarks):
            ...
            if result.is_valid:
                ...
                # Attempt recovery
                try:
                    recovered_data, recovery_messages = self.data_recovery.recover_partial_record(bookmark)
        ...
        summary = {
            ...
            'recovery_rate': len(recovered_bookmarks) / len(bookmarks) * 100 if bookmarks else 0,
            'success_rate': len(valid_bookmarks) / len(bookmarks) * 100 if bookmarks else 0,
    ...
    def generate_validation_report(self, validation_summary: Dict[str, Any]) -> str:
        ...
        if validation_summary['failed_bookmarks'] > 0:
            ...
            if failed_records:
                ...
                if len(failed_records) > 3:
                    report_parts.append(f"    ... and {len(failed_records) - 3} more failed records")
                report_parts.append("")
        ...
        if validation_summary['recovery_rate'] > 10:
            report_parts.append("  • Review data source quality - high recovery rate indicates input issues")
        ...
        if stats.get('warnings_issued', 0) > validation_summary['total_processed'] * 0.1:
            report_parts.append("  • Review warning messages for data quality improvements")
...
def validate_application_config(cli_args, config_path: Optional[Path] = None) -> Tuple[bool, List[str]]:
    """
...
def validate_and_process_csv_data(csv_data: List[Dict[str, Any]], 
                                 expected_columns: List[str],
                                 recovery_mode: bool = True) -> Tuple[List[Dict[str, Any]], str]:
    """
