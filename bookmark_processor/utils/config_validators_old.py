...
class ConfigurationSchema:
    ...
    def __init__(self):
        self.schema = {
            ...
            "executable": {
                "model_cache_dir": {
                    "type": "str",
                    "default": "~/.cache/bookmark-processor/models",
                },
    ...
    def get_option_schema(
        self, section_name: str, option_name: str
...
class ConfigValueValidator(Validator):
    ...
    def __init__(
        self, option_schema: Dict[str, Any], section_name: str, option_name: str
    ):
        """
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if expected_type == "str":
            ...
            result.add_error(
                f"Unknown type '{expected_type}' in schema", self.field_name
            )
    ...
    def _validate_string(
        ...
        if self.option_schema.get("sensitive", False):
            if value and len(value) < 10:
                ...
                result.add_warning(
                    "API key contains unusual characters", self.field_name
                )
    ...
    def _validate_integer(
        ...
        try:
            if isinstance(value, str):
                ...
                result.add_error(
                    f"Cannot convert {type(value).__name__} to integer", self.field_name
                )
                return result
        ...
        if min_val is not None and int_value < min_val:
            result.add_error(
                f"Value {int_value} is below minimum {min_val}", self.field_name
            )

        if max_val is not None and int_value > max_val:
            result.add_error(
                f"Value {int_value} is above maximum {max_val}", self.field_name
            )
    ...
    def _validate_float(self, value: Any, result: ValidationResult) -> ValidationResult:
        ...
        try:
            if isinstance(value, str):
                ...
                result.add_error(
                    f"Cannot convert {type(value).__name__} to float", self.field_name
                )
                return result
        except ValueError as e:
        ...
        if min_val is not None and float_value < min_val:
            result.add_error(
                f"Value {float_value} is below minimum {min_val}", self.field_name
            )

        if max_val is not None and float_value > max_val:
            result.add_error(
                f"Value {float_value} is above maximum {max_val}", self.field_name
            )
    ...
    def _validate_boolean(
        ...
        if isinstance(value, bool):
            ...
            result.add_error(
                f"Cannot convert {type(value).__name__} to boolean", self.field_name
            )
...
class ConfigurationFileValidator(Validator):
    ...
    def validate(self, config_path: Union[str, Path]) -> ValidationResult:
        ...
        if config_path is None:
            result.add_info(
                "No configuration file provided, using defaults", self.field_name
            )
        ...
        if not path.exists():
            result.add_error(
                f"Configuration file does not exist: {path}", self.field_name
            )
            return result

        if not path.is_file():
            result.add_error(
                f"Configuration path is not a file: {path}", self.field_name
            )
            return result
    ...
    def _validate_structure(
        ...
        for section_name in self.schema.required_sections:
            if not config.has_section(section_name):
                result.add_error(
                    f"Missing required section: [{section_name}]", self.field_name
                )
        ...
        for section_name in config.sections():
            if section_name not in known_sections:
                result.add_warning(
                    f"Unknown section: [{section_name}]", self.field_name
                )
        ...
        for section_name in config.sections():
            ...
            for option_name, value in config.items(section_name):
                ...
                if not option_schema:
                    result.add_warning(
                        f"Unknown option: {section_name}.{option_name}", self.field_name
                    )
                ...
                validator = ConfigValueValidator(
                    option_schema, section_name, option_name
                )
            ...
            for option_name, option_schema in section_schema.items():
                if (
                    not option_schema.get("optional", False)
                    and option_name not in sanitized_section
                ):
                    result.add_error(
                        f"Missing required option: {section_name}.{option_name}",
                        self.field_name,
                    )
    ...
    def _validate_cross_sections(
        ...
        if config.has_section("checkpoint") and config.has_section("executable"):
            checkpoint_dir = config.get(
                "checkpoint", "checkpoint_dir", fallback=".bookmark_checkpoints"
            )
            temp_dir = config.get(
            ...
            if checkpoint_path == temp_path:
                result.add_warning(
                    "Checkpoint directory and temp directory are the same",
                    self.field_name,
                )
        ...
        if config.has_section("logging"):
            ...
            performance_logging = config.getboolean(
                "logging", "performance_logging", fallback=True
            )

            if log_level == "DEBUG" and not performance_logging:
                result.add_info(
                    "DEBUG logging enabled but performance logging disabled",
                    self.field_name,
                )
    ...
    def create_default_config(self, output_path: Union[str, Path]) -> None:
        ...
        for section_name, section_schema in self.schema.schema.items():
            ...
            for option_name, option_schema in section_schema.items():
                ...
                if (
                    option_schema.get("optional", False)
                    and "default" not in option_schema
                ):
                    continue
