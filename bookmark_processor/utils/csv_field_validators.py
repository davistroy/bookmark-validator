    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if sanitized_id.startswith("-") or sanitized_id.endswith("-"):
                result.add_warning(
                    "ID should not start or end with hyphen", self.field_name
                )

            if "__" in sanitized_id:
                result.add_warning(
                    "ID contains consecutive underscores", self.field_name
                )
...
class BookmarkTitleValidator(StringValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if len(title) > 200:
                result.add_warning(
                    f"Title is quite long ({len(title)} chars)", self.field_name
                )

            # Check for suspicious content
            if title.count("\n") > 2:
                result.add_warning(
                    "Title contains multiple line breaks", self.field_name
                )
...
class BookmarkNoteValidator(StringValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if len(note) > 1000:
                result.add_warning(
                    f"Note is quite long ({len(note)} chars)", self.field_name
                )
...
class BookmarkExcerptValidator(StringValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if len(excerpt) > 500:
                result.add_warning(
                    f"Excerpt is quite long ({len(excerpt)} chars)", self.field_name
                )
...
class BookmarkFolderValidator(StringValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if folder:
                components = [
                    comp.strip() for comp in folder.split("/") if comp.strip()
                ]
                ...
                for i, component in enumerate(components):
                    if len(component) > 100:
                        result.add_warning(
                            f"Folder component '{component[:20]}...' is very long",
                            self.field_name,
                        )
                    ...
                    if component.startswith(".") or component.endswith("."):
                        result.add_warning(
                            f"Folder component '{component}' starts/ends with period",
                            self.field_name,
                        )
                ...
                if len(components) > 10:
                    result.add_warning(
                        f"Folder path has deep nesting ({len(components)} levels)",
                        self.field_name,
                    )
...
class BookmarkTagsValidator(Validator):
    ...
    def __init__(self):
        super().__init__(field_name="tags", required=False, allow_none=True)
        self.max_tags = 20
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if isinstance(value, list):
            ...
            result.add_error(
                f"Tags must be string or list, got {type(value).__name__}",
                self.field_name,
            )
            return result
        ...
        for i, tag in enumerate(tags):
            ...
            if len(tag) < 2:
                result.add_warning(
                    f"Tag '{tag}' is too short (< 2 chars)", self.field_name
                )
                continue

            if len(tag) > self.max_tag_length:
                result.add_warning(
                    f"Tag '{tag[:20]}...' is too long (> {self.max_tag_length} chars)",
                    self.field_name,
                )
            ...
            if not re.match(r"^[a-zA-Z0-9\s\-_.]+$", tag):
                result.add_warning(
                    f"Tag '{tag}' contains special characters", self.field_name
                )
                # Clean the tag
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value:
            ...
            if created_date.year < 1995:
                result.add_warning(
                    f"Created date is very old ({created_date.year})", self.field_name
                )
...
class BookmarkCoverValidator(StringValidator):
    ...
    def __init__(self):
        super().__init__(
            field_name="cover", required=False, allow_none=True, max_length=1000
        )
            ...
            if cover and (cover.startswith("http://") or cover.startswith("https://")):
                try:
                    ...
                    if any(path_lower.endswith(ext) for ext in image_extensions):
                        result.add_info(
                            "Cover appears to be an image URL", self.field_name
                        )
                    elif not path_lower or path_lower == "/":
                        result.add_warning(
                            "Cover URL points to root path", self.field_name
                        )

                    # Check for very long URLs
                    if len(cover) > 500:
                        result.add_warning(
                            f"Cover URL is very long ({len(cover)} chars)",
                            self.field_name,
                        )

                except Exception:
                    result.add_warning(
                        "Cover field contains invalid URL", self.field_name
                    )

            elif cover and not cover.startswith("http"):
                # Might be a relative path or other identifier
                result.add_info(
                    "Cover field does not appear to be a URL", self.field_name
                )
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.sanitized_value and isinstance(result.sanitized_value, str):
            ...
            if len(highlights) > 2000:
                result.add_warning(
                    f"Highlights field is very long ({len(highlights)} chars)",
                    self.field_name,
                )
...
class BookmarkFavoriteValidator(Validator):
    ...
    def __init__(self):
        super().__init__(field_name="favorite", required=False, allow_none=True)
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if isinstance(value, bool):
            ...
            if value_lower in ("true", "1", "yes", "on", "enabled", "y"):
                ...
                result.add_warning(
                    f"Unclear boolean value '{value}', defaulting to False",
                    self.field_name,
                )
                result.sanitized_value = False
        else:
...
class BookmarkCompositeValidator(CompositeValidator):
    ...
    def _validate_cross_fields(self, record: dict, result: ValidationResult) -> None:
        ...
        if not record.get("title") and not record.get("url"):
            result.add_error(
                "Bookmark must have either a title or URL", self.field_name
            )
        ...
        if not has_description:
            result.add_warning(
                "Bookmark has no descriptive content (title, note, or excerpt)",
                self.field_name,
            )
...
class CSVFieldValidator:
    ...
    def _validate_row_data(
        self, row_data: Dict[str, Any], expected_columns: List[str]
    ) -> ValidationResult:
