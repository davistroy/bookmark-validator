...
class RetryableItem:
    ...
    def add_attempt(self, error_type: ErrorType, error_message: str, 
                   response_code: Optional[int] = None, duration: float = 0.0) -> None:
...
class RetryHandler:
    ...
    ERROR_PATTERNS = {
        ErrorType.NETWORK_ERROR: [
            'network', 'unreachable', 'no route', 'host down'
        ],
        ErrorType.TIMEOUT: [
            'timeout', 'timed out', 'time out'
        ],
        ErrorType.DNS_ERROR: [
            'dns', 'name resolution', 'hostname', 'nodename'
        ],
        ErrorType.SSL_ERROR: [
    ...
    def __init__(self, default_max_retries: int = 3, default_base_delay: float = 2.0):
        ...
        logging.info(f"Initialized retry handler with max_retries={default_max_retries}")
    
    def add_failed_url(self, url: str, error: Exception, 
                      custom_max_retries: Optional[int] = None) -> None:
        """
        ...
        max_retries = (custom_max_retries or 
                      strategy.get('max_retries', self.default_max_retries))
        ...
        if max_retries > 0:
            ...
            logging.debug(f"Added {url} to retry queue (error: {error_type.value}, "
                         f"max_retries: {max_retries})")
        else:
            logging.debug(f"Not retrying {url} (error: {error_type.value}, no retries allowed)")
    
    def retry_failed_items(self, validator_func: Callable[[str], Any], 
        ...
        logging.info(f"Retry process completed. {len(successful_results)} URLs recovered.")
    ...
    def get_retry_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': successful_retries / total_items if total_items > 0 else 0.0
        }
    ...
    def _handle_retry_failure(self, item: RetryableItem, result: Any, duration: float) -> None:
        ...
        if item.should_retry:
            self.retry_queue.append(item)
            logging.debug(f"Re-queued {item.url} for retry "
                         f"(attempt {item.attempt_count}/{item.max_retries})")
        else:
            logging.debug(f"Giving up on {item.url} after {item.attempt_count} attempts")
    
    def _handle_retry_exception(self, item: RetryableItem, error: Exception, duration: float) -> None:
