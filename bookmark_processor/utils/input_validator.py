...
from bookmark_processor.utils.security_validator import SecurityValidator, SecurityValidationResult
...
class ValidationResult:
    ...
    def merge(self, other: 'ValidationResult') -> 'ValidationResult':
        ...
        result = ValidationResult(
            ...
            sanitized_value=other.sanitized_value if other.sanitized_value is not None else self.sanitized_value,
    ...
    def _check_required_and_none(self, value: Any) -> Optional[ValidationResult]:
        ...
        if self.required:
            if isinstance(value, str) and not value.strip():
                result.add_error("Field is required but received empty string", self.field_name)
                return result
            if isinstance(value, (list, dict)) and len(value) == 0:
                result.add_error("Field is required but received empty collection", self.field_name)
                return result
...
class StringValidator(Validator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        # Convert to string
        if not isinstance(value, str):
            result.add_info(f"Converting {type(value).__name__} to string", self.field_name)
            value = str(value)
        ...
        if self.min_length is not None and len(value) < self.min_length:
            result.add_error(f"String too short: {len(value)} < {self.min_length}", self.field_name)
        
        if self.max_length is not None and len(value) > self.max_length:
            result.add_error(f"String too long: {len(value)} > {self.max_length}", self.field_name)
        ...
        if self.allowed_values and value not in self.allowed_values:
            result.add_error(f"String '{value}' not in allowed values: {list(self.allowed_values)}", self.field_name)
...
class NumberValidator(Validator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if isinstance(value, (int, float)):
            ...
            result.add_error(f"Cannot convert {type(value).__name__} to number", self.field_name)
            return result
        ...
        if self.min_value is not None and numeric_value < self.min_value:
            result.add_error(f"Value {numeric_value} is below minimum {self.min_value}", self.field_name)
        
        if self.max_value is not None and numeric_value > self.max_value:
            result.add_error(f"Value {numeric_value} is above maximum {self.max_value}", self.field_name)
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if isinstance(value, datetime):
            ...
            if parsed_datetime is None:
                result.add_error(f"Cannot parse datetime '{value}' with any known format", self.field_name)
                return result
        ...
        if self.min_date and result.sanitized_value < self.min_date:
            result.add_error(f"Date {result.sanitized_value} is before minimum {self.min_date}", self.field_name)
        
        if self.max_date and result.sanitized_value > self.max_date:
            result.add_error(f"Date {result.sanitized_value} is after maximum {self.max_date}", self.field_name)
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if not parsed.scheme:
            ...
            try:
                ...
                result.add_error(f"Invalid URL after adding scheme: {e}", self.field_name)
                return result
        
        if parsed.scheme.lower() not in self.allowed_schemes:
            result.add_error(f"URL scheme '{parsed.scheme}' not allowed. Allowed: {list(self.allowed_schemes)}", self.field_name)
        ...
        if self.security_check and self.security_validator:
            ...
            if not security_result.is_safe:
                result.add_critical(f"Security validation failed: {security_result.blocked_reason}", self.field_name)
                result.metadata['security_issues'] = security_result.issues
            elif security_result.issues:
                result.add_warning(f"Security concerns: {', '.join(security_result.issues)}", self.field_name)
                result.metadata['security_issues'] = security_result.issues
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if not isinstance(value, list):
            if isinstance(value, str):
                ...
                result.add_info("Converted comma-separated string to list", self.field_name)
            elif hasattr(value, '__iter__'):
                items = list(value)
                result.add_info(f"Converted {type(value).__name__} to list", self.field_name)
            else:
                items = [value]
                result.add_info(f"Wrapped single {type(value).__name__} in list", self.field_name)
        else:
            items = value[:]  # Copy the list
        
        # Check length constraints
        if self.min_length is not None and len(items) < self.min_length:
            result.add_error(f"List too short: {len(items)} < {self.min_length}", self.field_name)
        
        if self.max_length is not None and len(items) > self.max_length:
            result.add_error(f"List too long: {len(items)} > {self.max_length}", self.field_name)
        ...
        if self.unique_items:
            ...
            validated_items = list(dict.fromkeys(validated_items))  # Preserve order while removing duplicates
            if len(validated_items) < original_length:
                duplicates_removed = original_length - len(validated_items)
                result.add_warning(f"Removed {duplicates_removed} duplicate items", self.field_name)
...
class CompositeValidator(Validator):
    ...
    def __init__(self, validators: List[Validator], 
        ...
        """
        super().__init__(field_name, False, True)  # Let individual validators handle requirements
...
def create_required_string(field_name: str, max_length: Optional[int] = None) -> StringValidator:
    """Create a required string validator"""
    return StringValidator(field_name=field_name, required=True, allow_none=False, max_length=max_length)


def create_optional_string(field_name: str, max_length: Optional[int] = None) -> StringValidator:
    """Create an optional string validator"""
    return StringValidator(field_name=field_name, required=False, allow_none=True, max_length=max_length)


def create_url_validator(field_name: str, required: bool = True) -> URLValidator:
    """Create a URL validator with security checks"""
    return URLValidator(field_name=field_name, required=required, allow_none=not required, security_check=True)
...
def create_datetime_validator(field_name: str, required: bool = False) -> DateTimeValidator:
    """Create a datetime validator"""
    return DateTimeValidator(field_name=field_name, required=required, allow_none=not required)
