...
class SecureLogger:
    ...
    SENSITIVE_PATTERNS = [
        # API Keys and tokens
        (re.compile(r'(?i)(api[_-]?key|token|secret|password|auth)["\s]*[:=]["\s]*([a-zA-Z0-9+/=_-]{10,})', re.IGNORECASE), r'\1=***REDACTED***'),
        (re.compile(r'(sk-[a-zA-Z0-9_-]{10,})', re.IGNORECASE), r'***API_KEY_REDACTED***'),
        (re.compile(r'(?i)(bearer\s+)([a-zA-Z0-9+/=]{10,})', re.IGNORECASE), r'\1***REDACTED***'),
        
        # AWS credentials
        (re.compile(r'(?i)(AKIA[0-9A-Z]{16})', re.IGNORECASE), r'***AWS_ACCESS_KEY_REDACTED***'),
        (re.compile(r'(?i)([A-Za-z0-9+/]{40})', re.IGNORECASE), r'***AWS_SECRET_REDACTED***'),
        
        # Common passwords in URLs
        (re.compile(r'(?i)(://[^:]+:)([^@]+)(@)', re.IGNORECASE), r'\1***PASSWORD_REDACTED***\3'),
        
        # Private IP addresses (for privacy)
        (re.compile(r'\b(?:192\.168|10\.|172\.(?:1[6-9]|2[0-9]|3[01]))\.[0-9]+\.[0-9]+\b'), r'***PRIVATE_IP***'),
        
        # Email addresses (for privacy)
        (re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'), r'***EMAIL_REDACTED***'),
        
        # Phone numbers
        (re.compile(r'\b(?:\+?1[-.\s]?)?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}\b'), r'***PHONE_REDACTED***'),
        
        # Credit card-like numbers
        (re.compile(r'\b[0-9]{4}[-\s]?[0-9]{4}[-\s]?[0-9]{4}[-\s]?[0-9]{4}\b'), r'***CARD_REDACTED***'),
        
        # Social Security-like numbers
        (re.compile(r'\b[0-9]{3}[-\s]?[0-9]{2}[-\s]?[0-9]{4}\b'), r'***SSN_REDACTED***'),
    ]
    
    # Additional patterns for URLs and file paths
    URL_PATTERNS = [
        # Query parameters that might contain sensitive data
        (re.compile(r'(?i)([?&](?:key|token|password|secret|auth)[=])([^&]+)', re.IGNORECASE), r'\1***REDACTED***'),
        
        # File paths that might contain usernames
        (re.compile(r'(?i)(\/users?\/|\/home\/|C:\\Users\\)([^\/\\]+)', re.IGNORECASE), r'\1***USER***'),
    ]
    ...
    def __init__(self, logger_name: str = __name__):
        ...
        self.compiled_patterns = [(pattern, replacement) for pattern, replacement in self.SENSITIVE_PATTERNS]
        self.compiled_url_patterns = [(pattern, replacement) for pattern, replacement in self.URL_PATTERNS]
    ...
    def log_url_validation_failure(self, 
                                  url: str, 
                                  reason: str, 
                                  security_issues: Optional[List[str]] = None) -> str:
        """
    ...
    def log_ssrf_attempt(self, 
                        url: str, 
                        blocked_reason: str, 
                        resolved_ips: Optional[List[str]] = None) -> str:
        """
...
def log_security_event(event_type: str, details: Dict[str, Any], severity: str = "warning") -> str:
