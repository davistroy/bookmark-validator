...
class CostTracker:
    ...
    def add_cost_record(
        ...
        if self.session_cost >= self.warning_threshold and self.session_cost - cost_usd < self.warning_threshold:
            self.logger.warning(f"Cost warning: Session cost has reached ${self.session_cost:.2f}")
    ...
    async def confirm_continuation(self) -> bool:
        ...
        try:
            ...
            if response in ['y', 'yes', '']:  # Empty response defaults to yes
                ...
                self.logger.info(f"User confirmed continuation at ${self.session_cost:.2f}")
                return True
            else:
    ...
    def get_cost_estimate(
        ...
        provider_records = [
            r for r in self.cost_records[-100:]  # Last 100 records
    ...
    def get_detailed_statistics(self) -> Dict[str, Any]:
        ...
        session_records = [
            ...
            if r.timestamp >= self.session_start_time
        ]
    ...
    def _load_cost_history(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.cost_records)} cost records, total: ${self.total_cost:.4f}")
