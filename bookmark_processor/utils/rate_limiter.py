...
class RateLimiter:
    ...
    def get_status(self) -> Dict[str, any]:
        ...
        utilization = (requests_in_window / self.requests_per_minute) if self.requests_per_minute > 0 else 0
...
class ServiceRateLimiters:
    ...
    def get_limiter(self, service: str) -> RateLimiter:
        ...
        if service not in self.limiters:
            raise ValueError(f"Unsupported service: {service}. Available: {list(self.limiters.keys())}")
        
        return self.limiters[service]
    
    def add_custom_limiter(self, service: str, requests_per_minute: int, burst_size: Optional[int] = None) -> None:
        """
        ...
        self.logger.info(f"Added custom rate limiter for {service}: {requests_per_minute} req/min")
