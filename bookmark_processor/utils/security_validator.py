...
class SecurityValidator:
    ...
    BLOCKED_SCHEMES = {
        'file', 'ftp', 'javascript', 'data', 'vbscript', 'mailto', 'tel',
        'sms', 'callto', 'webcal', 'wyciwyg', 'chrome', 'chrome-extension',
        'moz-extension', 'ms-browser-extension', 'about', 'blob'
    }
    ...
    def validate_url_security(self, url: str) -> SecurityValidationResult:
        ...
        risk_level = max(risk_level, hostname_result.risk_level, key=self._risk_priority)
    ...
    def _validate_port(self, port: Optional[int]) -> SecurityValidationResult:
        ...
        if port not in {80, 443, 8080, 8443}:
            return SecurityValidationResult(
                is_safe=True,
                risk_level='low',
                issues=[f'Non-standard port: {port}']
            )
    ...
    def _validate_path(self, path: str) -> SecurityValidationResult:
        ...
        return SecurityValidationResult(is_safe=True, risk_level=risk_level, issues=issues)
