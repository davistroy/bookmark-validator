...
Backward-compatible functional API for CLI argument validation.
These functions provide a simple interface for validating command-line arguments.
"""
        ...
        if not report["can_auto_detect"]:
            if "error" in report:
                ...
                raise ValidationError(
                    "No valid bookmark files found in current directory"
                )
...
def validate_ai_engine(ai_engine: str, config) -> str:
    """
...
def validate_bookmark_data(bookmark_data: dict) -> bool:
    ...
    """
    from .url import validate_url_format
    ...
    # Validate URL format
    if not validate_url_format(bookmark_data["url"]):
        raise ValidationError(f"Invalid URL format: {bookmark_data['url']}")
