...
class MemoryMonitor:
    ...
    def __init__(
        self, warning_threshold_mb: float = 3000, critical_threshold_mb: float = 3500
    ):
        """
...
class BatchProcessor(Generic[T, R]):
    ...
    def process_batches(
        ...
        for batch_start in range(0, self.total_items, self.batch_size):
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(
                        self.processed_count, self.total_items, f"Batch error: {e}"
                    )
                raise
    ...
    def process_all(
        ...
        for batch_results in self.process_batches(
