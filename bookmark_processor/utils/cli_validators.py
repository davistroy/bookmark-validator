...
class PathValidator(Validator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if self.allowed_extensions and path.suffix.lower() not in self.allowed_extensions:
            result.add_error(f"File extension '{path.suffix}' not allowed. Allowed: {list(self.allowed_extensions)}", self.field_name)
            ...
            if not parent.exists():
                if self.create_parent_dirs:
                    try:
                        ...
                        result.add_info(f"Created parent directories: {parent}", self.field_name)
                    except Exception as e:
                        result.add_error(f"Cannot create parent directories: {e}", self.field_name)
                        return result
                else:
                    result.add_error(f"Parent directory does not exist: {parent}", self.field_name)
                    return result
            
            if self.must_be_writable and not os.access(parent, os.W_OK):
                result.add_error(f"Parent directory is not writable: {parent}", self.field_name)
        ...
        if result.is_valid and result.sanitized_value:
            ...
            try:
                ...
                if file_size == 0:
                    ...
                    result.add_warning(f"Input file is very large ({file_size // (1024*1024)} MB)", self.field_name)
            except Exception as e:
            ...
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    ...
                    if not first_line:
                        result.add_error("Input file appears to be empty", self.field_name)
                    elif ',' not in first_line and ';' not in first_line and '\t' not in first_line:
                        result.add_warning("File may not be in CSV format (no delimiters found)", self.field_name)
            except Exception as e:
                result.add_warning(f"Cannot read file for format check: {e}", self.field_name)
...
class OutputFileValidator(PathValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.is_valid and result.sanitized_value:
            ...
            if path.exists():
                result.add_warning(f"Output file exists and will be overwritten: {path}", self.field_name)
            ...
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    ...
                    if '[' not in content or ']' not in content:
                        result.add_warning("Configuration file may not be in INI format", self.field_name)
            except Exception as e:
                result.add_warning(f"Cannot read config file for format check: {e}", self.field_name)
...
class MaxRetriesValidator(NumberValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.is_valid and result.sanitized_value is not None:
            ...
            # Provide recommendations
            if max_retries == 0:
                result.add_warning("Zero retries may result in many failed URLs", self.field_name)
            elif max_retries > 5:
                result.add_warning("High retry count may significantly slow processing", self.field_name)
...
class AIEngineValidator(StringValidator):
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        if result.is_valid and result.sanitized_value:
            ...
            if engine == 'local':
                result.add_info("Using local AI models (no API key required)", self.field_name)
            elif engine in ['claude', 'openai']:
                result.add_info(f"Using {engine} cloud API (requires API key in config)", self.field_name)
...
class ArgumentCombinationValidator(Validator):
    ...
    def validate(self, args_dict: Dict[str, Any]) -> ValidationResult:
        ...
        if input_path and output_path and Path(input_path).resolve() == Path(output_path).resolve():
            result.add_error("Input and output files cannot be the same", self.field_name)
    ...
    def validate_all_arguments(self, args: argparse.Namespace) -> ValidationResult:
        ...
        for arg_name, validator in self.validators.items():
            if hasattr(args, arg_name):
                ...
                validated_args[f"{arg_name}_path" if arg_name in ['input', 'output', 'config'] else arg_name] = arg_result.sanitized_value
...
def validate_cli_arguments(args: argparse.Namespace) -> Tuple[Dict[str, Any], Optional[str]]:
    """
