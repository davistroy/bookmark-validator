...
class AIFolderGenerator:
    ...
    def _init_category_patterns(self) -> None:
        ...
        self.category_patterns = {
            'Development': {
                'keywords': ['programming', 'coding', 'developer', 'software', 'github', 'api', 'framework'],
                'domains': ['github.com', 'stackoverflow.com', 'dev.to'],
                'subcategories': {
                    'Frontend': ['react', 'vue', 'angular', 'css', 'html', 'javascript', 'typescript'],
                    'Backend': ['python', 'java', 'node', 'django', 'flask', 'api', 'database'],
                    'DevOps': ['docker', 'kubernetes', 'aws', 'azure', 'ci/cd', 'jenkins'],
                    'Mobile': ['android', 'ios', 'react native', 'flutter', 'swift']
                }
            },
            'AI & Machine Learning': {
                'keywords': ['ai', 'artificial intelligence', 'machine learning', 'deep learning', 'neural', 'model'],
            ...
            'Design': {
                'keywords': ['design', 'ux', 'ui', 'graphic', 'typography', 'color', 'layout'],
            ...
            'Business': {
                'keywords': ['business', 'startup', 'entrepreneur', 'marketing', 'finance', 'strategy'],
            ...
            'Education': {
                'keywords': ['tutorial', 'course', 'learn', 'education', 'training', 'guide'],
            ...
            'Tools & Resources': {
                'keywords': ['tool', 'utility', 'resource', 'generator', 'converter', 'calculator'],
            ...
            'Reference': {
                'keywords': ['documentation', 'reference', 'wiki', 'encyclopedia', 'dictionary'],
                'domains': ['wikipedia.org', 'docs.python.org', 'developer.mozilla.org'],
                'subcategories': {
    ...
    def generate_folder_structure(self,
                                ...
                                original_folders_map: Dict[str, str] = None) -> FolderGenerationResult:
        """
        ...
        logging.info(f"Generated {result.total_folders} folders with max depth {result.max_depth}")
    ...
    def _categorize_bookmarks(self,
                            ...
                            original_folders_map: Dict[str, str]) -> Dict[str, List[Tuple[Bookmark, str]]]:
        """
    ...
    def _determine_category(self,
                          ...
                          original_folder: str) -> Tuple[str, str]:
        """
        ...
        if original_folder:
            ...
            if parts and parts[0]:
                ...
                for category in self.category_patterns:
                    if folder_name.lower() in category.lower() or category.lower() in folder_name.lower():
                        return category
    ...
    def _build_folder_hierarchy(self, 
        ...
        for category, bookmark_list in categorized_bookmarks.items():
            ...
            if len(subcategory_groups) > 1 or len(bookmark_list) > self.max_bookmarks_per_folder:
    ...
    def _split_large_folders(self, folder: FolderNode, depth: int = 0) -> None:
        ...
        for child in list(folder.children):  # Use list() to avoid modification during iteration
        ...
        if len(folder.bookmarks) > self.max_bookmarks_per_folder:
            ...
            if all(len(bookmarks) > self.max_bookmarks_per_folder for bookmarks in groups.values()):
            ...
            for i, (group_name, group_bookmarks) in enumerate(groups.items()):
                if len(group_bookmarks) > 0:
                    ...
                    subfolder = FolderNode(
                        name=subfolder_name,
                        path=f"{folder.path}/{subfolder_name}"
                    )
    ...
    def _group_similar_bookmarks(self, bookmarks: List[Bookmark]) -> Dict[str, List[Bookmark]]:
    ...
    def _simple_numeric_split(self, bookmarks: List[Bookmark]) -> Dict[str, List[Bookmark]]:
    ...
    def _collect_assignments(self, folder: FolderNode, assignments: Dict[str, str]) -> None:
    ...
    def _add_folder_lines(self, folder: FolderNode, lines: List[str], indent: str) -> None:
