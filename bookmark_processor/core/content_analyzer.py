...
class ContentAnalyzer:
    ...
    CONTENT_CATEGORIES = {
        'article': [
            'article', 'blog', 'post', 'news', 'story', 'entry',
            'editorial', 'feature', 'column', 'review'
        ],
        'documentation': [
            'docs', 'documentation', 'manual', 'guide', 'tutorial',
            'reference', 'api', 'readme', 'help', 'wiki'
        ],
        'academic': [
            'paper', 'research', 'study', 'journal', 'thesis',
            'publication', 'academic', 'scholar', 'arxiv'
        ],
        'video': [
            'youtube', 'vimeo', 'video', 'watch', 'movie', 'film',
            'tv', 'series', 'episode', 'stream'
        ],
        'social': [
            'twitter', 'facebook', 'linkedin', 'instagram', 'reddit',
            'social', 'forum', 'discussion', 'community'
        ],
        'tool': [
            'tool', 'utility', 'app', 'application', 'software',
            'service', 'platform', 'generator', 'converter'
        ],
        'repository': [
            'github', 'gitlab', 'bitbucket', 'repo', 'repository',
            'code', 'source', 'project', 'library', 'framework'
        ],
        'commerce': [
            'shop', 'store', 'buy', 'sell', 'product', 'price',
            'amazon', 'ebay', 'marketplace', 'ecommerce'
        ]
    }
    ...
    IGNORE_SELECTORS = [
        ...
        '.social', '.share', '.comment-form', '.newsletter',
    ...
    def __init__(self, 
        ...
        logging.info(f"Initialized content analyzer (timeout={timeout}s, "
                    f"max_length={max_content_length})")
    
    def analyze_content(self, url: str, existing_title: str = "", 
                       existing_note: str = "", existing_excerpt: str = "") -> ContentData:
        """
        ...
        try:
            ...
            content_data.reading_time_minutes = self._calculate_reading_time(content_data.word_count)
    ...
    def _extract_with_beautifulsoup(self, html_content: str, content_data: ContentData) -> ContentData:
    ...
    def _extract_with_regex(self, html_content: str, content_data: ContentData) -> ContentData:
    ...
    def _enhance_with_existing_data(self, content_data: ContentData, 
                                   existing_title: str, existing_note: str, 
                                   existing_excerpt: str) -> ContentData:
        """Enhance content data with existing bookmark information"""
        # Use existing title if better or missing
        if existing_title and (not content_data.title or len(existing_title) > len(content_data.title)):
