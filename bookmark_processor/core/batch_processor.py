...
class BatchProcessor:
    ...
    def _print_final_statistics(self, statistics: Dict[str, Any]) -> None:
        ...
        """
        print(f"\n📊 Processing Complete!")
        print(f"  ✅ Processed: {statistics['processed_count']}/{statistics['total_bookmarks']}")
        ...
        if session_cost > 0:
            ...
            print(f"  📈 Success Rate: {session_info.get('success_rate_percent', 0.0):.1f}%")
            ...
            if providers:
                ...
                for provider, provider_stats in providers.items():
                    print(f"     {provider}: ${provider_stats['cost_usd']:.4f} ({provider_stats['requests']} requests)")
