...
class ChromeHTMLGenerator:
    ...
    def _build_folder_hierarchy(self, bookmarks: List[Bookmark]) -> FolderNode:
        ...
        for bookmark in bookmarks:
            ...
            if bookmark.folder:
                ...
                if path_parts[0].lower() in ["bookmarks bar", "bookmarks_bar", "toolbar"]:
    ...
    def _log_folder_stats(self, root: FolderNode) -> None:
        ...
        self.logger.info(f"Folder statistics: {total_folders} folders, {total_bookmarks} bookmarks")
    ...
    def _generate_folder_html(self, folder: FolderNode, is_top_level: bool = False) -> List[str]:
        """
        ...
        folder_tag = f'<H3 {" ".join(folder_attrs)}>{self._escape_html(folder.name)}</H3>'
        ...
        for child_folder in sorted(folder.children.values(), key=lambda f: f.name.lower()):
