...
class DuplicateDetector:
    ...
    def detect_duplicates(self, bookmarks: List[Bookmark]) -> Dict[str, DuplicateGroup]:
        ...
        duplicate_groups = {
            url: group for url, group in url_groups.items() 
    ...
    def resolve_duplicates(self, 
                          ...
                          strategy: str = "newest") -> Dict[str, DuplicateGroup]:
        """
    ...
    def process_bookmarks(self, 
                         ...
                         dry_run: bool = False) -> Tuple[List[Bookmark], DuplicateDetectionResult]:
        """
        ...
        duplicates_count = sum(len(group.bookmarks) - 1 for group in duplicate_groups.values())
        ...
        if not dry_run and duplicate_groups:
            ...
            # Filter bookmarks
            deduplicated = [b for b in bookmarks if id(b) not in bookmarks_to_remove_ids]
        else:
            ...
            for i, group in enumerate(result.duplicate_groups, 1):
                ...
                if kept:
                    report_lines.extend([
                        ...
                        f"    Note: {kept.note[:50]}..." if kept.note and len(kept.note) > 50 else f"    Note: {kept.note}",
                        ""
                    ])
